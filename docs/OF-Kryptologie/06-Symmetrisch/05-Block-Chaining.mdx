...
4. Decodieren Sie die Pentacode-codierte Geheimtext-Bitfolge zu einer Zeichenfolge.
    <Answer label="c" type="string" id="8f90991c-64f0-41d3-be67-68a2d0af1982" solution="RD@DVENTE GUGIXU"/>
</ProgressState>
::::
...
<Answer label="IV" inputWidth="9em" labelWidth="2em" type="string" id="791d6b06-0ab1-4cb7-82d2-388ed8cd7626" solution="00110" sanitizer={(val) => val.trim().slice(0, 4)} monospace/>
<i>Nach XOR mit IV </i>
...
<Answer label="Text (nicht binär)" fullWidth  type="string" id="4ce989c5-27ad-4146-bdf6-adb7b5908b55" solution="DOLCE" sanitizer={(val) => (/^\s\S/.test(val || '') && val?.length === 8) ? val?.toUpperCase().slice(2) : UPPER_NOSPACE(val)?.length === 7 ? UPPER_NOSPACE(val).slice(2) : UPPER_NOSPACE(val)} monospace/>
::::
