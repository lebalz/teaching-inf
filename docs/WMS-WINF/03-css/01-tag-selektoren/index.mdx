...
::::cards{columns=3}
<HtmlEditor
  htmlTransformer={(css) => `<style>${css}</style><body>${example2}</body>`}
  hideWarning
  title='styles-1.css'
  lang='scss'
  minLines={8}
...
::br{flexBasis="45%" maxWidth="45%" zIndex="199"}

<ProgressState id="b7b34f3b-7463-40fb-821a-603aac543fe5" confirm preventTogglingFutureSteps>
...
9. Optimierungen: bei den Listen (`ul` und `ol`) kann der Zusätzliche Abstand nach Links noch etwas reduziert werden:
    ...
    ```
    :::insight[Bemerke]
    Mit der Komma-Separierung `ul, ol` kann dieselbe Regel auf mehrere Tags angewandt werden.
    :::
