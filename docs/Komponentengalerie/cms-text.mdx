...
Beim `WithCmsText` wird standardmässig ein Upload-Formular für Admins angezeigt. Dieses Formular ermöglicht das Hochladen von Texten für die angegebene DocumentRootId (oder im Falle eines `WithCmsText`s auch für mehrere DocumentRootIds). Für einzelne CMS-Texte kann das Formular mit `showActions` angezeigt werden.
