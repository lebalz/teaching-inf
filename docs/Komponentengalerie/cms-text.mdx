...
<CodeEditor name="hello" onChange={(c) => console.log(c)} />
...
Um CMS-Texte hochzuladen, kann mit dem Attribut `showActions` ein Upload-Formular für Admins angezeigt. Dieses Formular ermöglicht das Hochladen von Texten für die angegebene DocumentRootId (oder im Falle eines `WithCmsText`s auch für mehrere DocumentRootIds).
