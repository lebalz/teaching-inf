---
page_id: 7c874061-5773-4b07-abd0-cd5630403e10
---
import CmsLink from '@tdev-components/documents/CmsText/Link';
import Steps from '@tdev-components/Steps';
import TaskState from '@tdev-components/documents/TaskState';
import PermissionsPanel from '@tdev-components/PermissionsPanel'
import BrowserWindow from '@tdev-components/BrowserWindow'
import { observer } from 'mobx-react-lite';
import { useStore } from '@tdev-hooks/useStore';

export const NotenLabel = observer(() => {
    const docRootStore = useStore('documentRootStore');
    const userStore = useStore('userStore');
    const documentRoot = docRootStore.find('5f64b284-1eb3-46a9-9176-efea2ffa6391');
    const defaultText = 'Meine Note soll zählt.*';
    if (!documentRoot) {
        return defaultText;
    }
    const { firstMainDocument } = documentRoot;
    if (!firstMainDocument) {
        return defaultText;
    }
    if (firstMainDocument.taskState === 'unset') {
        return <span>Die Note für <i>{userStore.viewedUser.name}</i> <b>zählt nicht!*</b></span>;
    }
    return <span>Die Note für <i>{userStore.viewedUser.name}</i> <b>zählt 0.5.*</b></span>;
});

# Projekt

:::info[Rahmenbedingungen]
Benotung
: Die Note zählt halb ($0.5$)
Verfügbare Zeit
: 3 Doppellektionen
Abgabe
: Die Abgabe erfolgt direkt über [students-wiki.gbsl.website](https://students-wiki.gbsl.website/)
Datum
: 20.06.2025, 22:00 Uhr
:::

::::info[Projektabgabe]
Die Abgabe erfolgt über [students-wiki.gbsl.website](https://students-wiki.gbsl.website/).
1. Stellen Sie sicher, dass Sie bei Ihrer Abgabe als **Autor:in** eingetragen sind.
2. Teilen Sie das Kapitel direkt aus ihrem **persönlichen** Bereich mit B. Hofer, indem Sie auf __Teilen__ klicken:
    <BrowserWindow bodyStyle={{ padding: 0, lineHeight: 0}}>
        ::video[./images/Abgabe-Projekt.mp4]{borderRadius=4px}
    </BrowserWindow>
3. Geben Sie hier an, ob die Note zählt oder nicht - ab 22:00 Uhr am 20.6.2025 kann die Gewichtung nicht mehr geändert werden.

    <TaskState states={['checked', 'unset']} id="5f64b284-1eb3-46a9-9176-efea2ffa6391" inline /> <NotenLabel />
    <PermissionsPanel documentRootId="5f64b284-1eb3-46a9-9176-efea2ffa6391"/>

    :::danger[\* Abgabe ist obligatorisch]
    Die Abgabe ist obligatorisch. Wird keine Abgabe gemacht oder sind die formalen Mindest-Anforderungen nicht erfüllt (bspw. Abgabe eines leeren Dokuments oder es fehlen Quellenangaben oder Unterkapitel fehlen oder Quizz-Fragen nicht vorhanden oder kein Bild, oder Inhalt passt nicht zur abgegebenen Proposal), so wird eine halb zählende **Note 2.0** vergeben.
    :::

::::

## Projektbeschreibung

:::info[Ziel]
Bei diesem Projekt werden mehrere Ziele verfolgt:
- **Erlernen** von KI-Tools
    - Erforschen von KI-Tools
    - Gewinnbringender Einsatz von KI-Tools
    - Verfassen des bewerteten Beitrags mit KI-Tools
- Eintauchen in ein Thema aus dem Bereich "Big Data und/oder KI/Maschinelles Lernen"
    - Fokus auf ein kleines Teilgebiet (selbständige Wahl)
    - Mehr erfahren zu diesem Teilgebiet
    - Selbst gelegte Lernziele erreichen
- **Erstellen** einer Lerneinheit (Kapitel in einem Buch)
    - Zielgruppe: Mitschüler:innen (Gym 1)
    - Ihre Lerneinheit:
        - Weist folgende Struktur auf:
            - **Kurzüberblick** über die Einheit. In 3-5 Sätzen wird das Thema und was gelernt werden kann, umrissen.
            - **Einleitung**: Hier wird das Thema vorgestellt - die Alltagsrelevanz, allenfalls die geschichtliche Entwicklung und der Bezug zu den Themen "Big Data und/oder KI/Maschinelles Lernen" wird hergestellt.
            - **Hauptteil**: Hier wird das Thema vertieft. Alle relevanten Begriffe werden erklärt und der Kern Ihres Themas wird präsentiert. Fachliche Korrektheit ist zentral, weshalb mindestens drei Quellenangaben zu realen PDFs oder Büchern angegeben werden werden. Sprachlich beschränkt sich das Kapitel aufs Wesentliche und erklärt präzise und prägnant ohne dabei ausschweifende Formulierungen zu verwenden.
            - **Zusammenfassung**: Hier wird das Thema zusammengefasst und die wichtigsten Punkte werden nochmals aufgelistet. Allenfalls wird ein Ausblick auf die Zukunft gegeben.
            - **Quiz**: Hier werden drei Fragen gestellt, die das Thema zusammenfassen. Die Fragen sind so formuliert, dass sie mit dem Wissen aus dem Kapitel beantwortet werden können. Die Antworten sind in einem separaten Abschnitt angegeben.
            - **Quellen**: Hier werden die Quellen angegeben, die für das Kapitel verwendet wurden. Diese sind in einem einheitlichen Format (z.B. APA) angegeben. Zudem werden alle KI's angegeben, die für die Erstellung des Kapitels verwendet wurden.
        - Beinhaltet mindestens ein Bild.
        - Beschränkt sich auf das Wesentliche (nicht mehr als ca. zwei Seiten).
    - Die Lerneinheit wird in einem Wiki erstellt und ist zum Schluss für alle Mitschüler:innen zugänglich.
    - **Bevor** Sie mit dem Erstellen der Lerneinheit beginnen, müssen Sie sich mit dem Thema auseinandersetzen und die wichtigsten Begriffe und Konzepte verstehen. Dies geschieht durch das Lesen von Artikeln, Büchern und anderen Quellen. Sie können auch Videos oder Podcasts nutzen, um sich in das Thema einzuarbeiten.
    - **KI**: Die Lerneinheit darf/soll mit Hilfe von KI-Tools erstellt werden. Wichtig dabei ist, dass Sie die KI-Tools als Hilfsmittel nutzen und nicht einfach nur den Text kopieren. Sie sollten die Texte, die Sie von der KI erhalten, kritisch hinterfragen und gegebenenfalls anpassen.
- **Proposal**: Sie müssen ein Proposal (Vorschlag) für Ihre Lerneinheit erstellen. Dieses Proposal sollte folgende Punkte enthalten:
    - Titel der Lerneinheit
    - Kurzbeschreibung des Themas - was wird behandelt, welchen Fokus setzen Sie, was ist Ihnen wichtig?
    - Lernziele, die Sie selber erreichen möchten
    - Zeitplan für die Erstellung der Lerneinheit
:::


<Steps>
1. Individuelles Projekt finden
    - Lesen Sie die Erfahrungsberichte im verlinkten [PDF](https://erzbe-my.sharepoint.com/:b:/g/personal/balthasar_hofer_gbsl_ch/EcqH-hlKTztGl8aCxn7XwH0BdKudcpFbOPYuXBHsDN6ljw?e=1C4tGg) durch. **Die im Text erwähnten Aufgaben müssen nicht gemacht werden.**
    - Versuchen Sie verschiedene KI-Tools aus, um damit ein für Sie spannendes und anregendes Themengebiet zu finden. Das Oberthema ist vorgegeben und heisst "Big Data und/oder KI/Maschinelles Lernen".
2. Rahmenbedingungen klären
    - Wussten Sie, dass Sie der KI mitgeben können, wie präzise Sie die Antwort haben möchten? Das nennt sich "Temperature"... Lesen [hier](https://erzbe-my.sharepoint.com/:b:/g/personal/balthasar_hofer_gbsl_ch/EUCPSprJgjdHieuCNOLanpQB50f07mr2aaqcjPOnrpg6sA?e=HGwPQL) mehr dazu.
    - Formulieren Sie persönliche Erwartungen zum Thema.
    - Geben Sie konkrete Ziele an, die Sie erreichen möchten
        - Lernziele, die Sie sicher erreichen möchten
        - Lernziele, bei denen Sie unsicher sind, ob Sie sie in den 3 Doppellektionen erreichen können.
        - Optionale Lernziele, "nice to have"
    - Definieren Sie den Projektumfang.
    - Abgabe unter [students-wiki.gbsl.website](https://students-wiki.gbsl.website/) bis heute nach der Lektion.
3. Lernen und Projekt umsetzen
    - KI's können auch gerne mal flunkern 😒 Wie gehen Sie damit um, was ist wichtig dabei? Lesen Sie dazu [mehr](https://erzbe-my.sharepoint.com/:b:/g/personal/balthasar_hofer_gbsl_ch/EYDqchlHpCZOrfeO0ExvspAB98ghnhL_fNti165vyvHuRA?e=ZxvMRE).
    - Zeit einzutauchen in das Thema
        - Recherchieren Sie zu Ihrem Thema
        - Suchen Sie Quellen und Literatur
4. Projekt abgeben
</Steps>


## Eine Auswahl an KI-Tools [^1]
:::cards{flexBasis=300px}
### Texte **verfassen**
- [ChatGPT](https://chatgpt.com/)
- [Gemini](https://gemini.google.com/)
- [Microsoft Copilot](https://copilot.microsoft.com/chats)
- [Anthropic Claude](https://claude.ai/login?returnTo=%2F%3F)
- [Poe](https://poe.com/login)
- [Perplexity](https://www.perplexity.ai/)
- [Genie (App)](https://usegenie.ai/)

::br
### Texte **weiterschreiben**
- [Jenni](https://jenni.ai/)
- [Aithor](https://aithor.com/de-de)
::br
### Texte **erklären**
- [ChatPDF](https://www.chatpdf.com/de)
- [SciSpace auf Typeset.io](https://scispace.com/)
- [Explainpaper](https://www.explainpaper.com/)
- [Humata](https://www.humata.ai/)
::br
### Bibliographien **suchen**
- [Perplexity](https://www.perplexity.ai/)
- [Microsoft Copilot](https://copilot.microsoft.com/chats)
- [Connected Papers](https://www.connectedpapers.com/)
- [Elicit](https://elicit.com/)
- [Liftmaps](https://www.litmaps.com/)
- [ResearchRabbit](https://www.researchrabbit.ai/)
- [Jenni](https://jenni.ai/)
- [Aithor](https://aithor.com/de-de)
::br
### Texte **korrigieren**
Texte direkt während des Schreibens korrigieren

- [DeepL Write (Deutsch und Englisch)](https://www.deepl.com/de/write)
- [Duden-Mentor (Deutsch)](https://mentor.duden.de/)
- [Grammarly (Englisch)](https://www.grammarly.com/)
- [EditGPT (verschiedene Sprachen)](https://editgpt.app/)
- [Microsoft Editor (verschiedene Sprachen)](https://www.microsoft.com/de-ch/microsoft-365/microsoft-editor?market=ch)
- [Paperpal (Englisch)](https://paperpal.com/)
- [Scribbr (Englisch)](https://www.scribbr.ch/)
::br
### **Feedback** zu Texten
- [PEER (Paper Evaluation and Empowerment Resource)](https://peer-ai-tutor.streamlit.app/)
- [Fiete.ai](https://www.fellofish.com/)
::br
### **Texte** übersetzen
- [DeepL](https://www.deepl.com/de/translator)
- [ChatGPT](https://chatgpt.com/)
- [Google Translate](https://translate.google.com/?sl=auto&tl=de&op=translate)
- [Textshuttle](https://www.supertext.com/de-CH/textshuttle-heisst-neu-supertext)
::br
### **Texte** transkribieren
- [Töggl](https://xn--tggl-5qa.ch/)
- [Cockatoo](https://www.cockatoo.com/)
:::

[^1]: Basiert auf [Es Macht Klick](https://www.hep-verlag.ch/klick) vom hep Verlag.