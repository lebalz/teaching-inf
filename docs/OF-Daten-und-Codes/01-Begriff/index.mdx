...
:::warning[Code $\neq$ Verschlüsselung]
Ein Code ist nicht dasselbe wie eine Verschlüsselung. Bei einer Verschlüsselung wird die Information so verändert, dass sie nur mit einem Schlüssel wieder entschlüsselt werden kann. Bei einem Code ist die Umwandlung festgelegt und kann ohne Schlüssel rückgängig gemacht werden.
:::
::::
...
export const QrCode = () => {
    ...
    return (
        <Generator withInput size="30em" text={user?.name || 'Hallo Welt'} />
    )
