...
## Einführung
Im vorhergehenden Kapitel haben Sie Quadrate gezeichnet, deren Seitenlänge im Programm **fest eingebaut** waren. Manchmal sollen diese Werte aber auch **dynamisch festgelegt** werden - bspw. durch eine Benutzereingabe. Dazu muss das Programm die eingegebene Zahl als __Variable speichern__.
...
![Variable Zuweisen --width=300px](./images/assign-variable.webp)


Mit der Schreibweise `anzahl = 12` wird eine solche Box erstellt. In Zukunft sagen wir, dass damit die Variable `anzahl` **definiert** und ihr einen **Wert zugewiesen** wird. Um den Inhalt der Box zu erhalten, kann der Name (ohne Anführungszeichen und ohne Klammern) geschrieben werden. Im Struktogramm werden alle Variablen **umrahmt**.
...
![Variablen überschreiben --width=350px](./images/reassign-variable.webp)
...
- ausgegebene Zahl `=>`
- aktuell gespeicherter Wert in der Variable `anzahl` mit `anzahl=...`
...
Mathematik
: `=` bedeutet "ist gleich"
: Es handelt sich um eine Gleichung, die Reihenfolge spielt keine Rolle.
: $x + 2 = 12$ ist dasselbe wie $12 = x + 2$.
Programmierung
: `=` bedeutet "zuweisen"
: Der rechtsstehende Wert wird der linksstehenden Variable zugewiesen, die Reihenfolge ist **entscheidend**.
: ✅ `anzahl = 12` beschreibt eine Zuweisung.
: ❌ `12 = anzahl` ist ein Syntaxfehler.
</div>
:::
...
<Strukto program={[
    ...
    {type: 'step', code: <span><span className="var">bmi</span> = <span className="var">gewicht</span> geteilt durch das Produkt aus <span className="var">grösse</span> mal <span className="var">grösse</span></span>},
...
```
→ Der eingegebene Text kann nicht multipliziert werden - zuerst müssen `gewicht` und `grösse` in Dezimalzahlen (`float`) umgewandelt werden:
```py
...
```py live_py slim
from turtle import *
n = input('Wie viele Ecken soll das n-Eck haben?')
n = int(n)
