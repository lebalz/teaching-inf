...
Befehle
: oder `Unterprogramme`, `Funktionen`
...
Liegt ein Fehler vor, der nicht bei Schlüsselwörtern (oder deren Anordnung) vorliegt, so handelt es sich oft um einen Namens-Fehler (`NameError`). Es wird also versucht, ein Unterprogramm zu öffnen (oder eine Variable abzufragen), das aber **unter diesem Namen nicht gefunden** werden kann.
