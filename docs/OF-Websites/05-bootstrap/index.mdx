...
- Die Rezepte-Leiset soll ebenfalls verlinkt werden
...
Rezept
: Aktuell im `index.html`-File, wird neu in __`rezepte.html`__ umbenannt.
CV-Seite
...
:::warning[Bemerke]
Der Link führt auf die Datei `rezepte.html` im gleichen Ordner
- da der Browser automatisch die Dateiendung `.html` ergänzt, muss diese nicht explizit angegeben werden.
- der `.` bei `./rezepte` bedeutet, dass die Datei im gleichen Ordner, in dem sich auch die aktuelle Datei befindet, gesucht wird. Wird das `.` weggelassen, wird die Datei im sog. Rootverzeichnis gesucht (in diesem Ordner liegt die Hauptseite, bei uns das `index.html`).
:::
