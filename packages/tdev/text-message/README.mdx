...
<CodeShowcase imports={[`import SimpleChat from '@tdev/text-message/components/SimpleChat';`]}>
    <SimpleChat id="1c762ae7-b3f2-4379-a779-3ac4b7858eb4" name="Allgemeiner Chat" maxHeight='300px' />
</CodeShowcase>


<CodeShowcase imports={[`import DynamicDocumentRoots from '@tdev-components/documents/DynamicDocumentRoots';`]}>
...
:::note[Berechtigungen]
Bei der Erstellung eines `dynamic_document_roots` werden die Berechtigungen auf `access=RO_DocumentRoot` und `sharedAccess=RW_DocumentRoot` gesetzt - was vernünftige Standards sind. Zudem wird automatisch eine __RW__ Berechtigung für die erstellende Admin-User:in gesetzt.
:::
