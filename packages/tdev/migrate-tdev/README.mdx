...
`updateTdev [[--format]]`
: führt `yarn run updateTdev` aus, installiert mögliche Package-Updates und committet+pushed die Änderungen.
: Optional kann `--format` angegeben werden, um das Projekt nach dem Update zu formatieren.
`reinstall`
