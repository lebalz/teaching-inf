---
page_id: 0c025b33-7501-4fff-bf19-667c48fe4560
...
Hier werden die fünf häufigsten Fehler in Python zusammengefasst.
...
Beispiel: Alle Codezeilen, die gemeinsam wiederholt werden sollen, sind gleich weit eingerückt:
```py live_py slim
print('--- Start ---')
for i in range(2):
    print('🥳')      # \ 
...
## 5. Unterprogramme aufrufen

Damit ein Unterprogramm aufgerufen wird, müssen die Klammern `()` geschrieben werden - auch wein kein Parameter übergeben wird.
Wenn man die Klammern weglässt, ist dies immer noch korrektes Python:

```py live_py slim
from turtle import *
### PRE
penup # korrekter Code, aber kein Aufruf des Unterprogramms
penup() # penup wird aufgerufen
```