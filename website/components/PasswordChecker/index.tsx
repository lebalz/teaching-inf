...
const PasswordChecker = () => {
    ...
    const [checks, setChecks] = React.useState<{ level: keyof typeof LEVEL_MAP }[]>([]);
    ...
    React.useEffect(() => {
        ...
        const options = {
            ...
            outputChecks: (checks: { level: keyof typeof LEVEL_MAP }[], input: string) => {
                let level = 0;
    ...
    React.useEffect(() => {
        ...
        const z = (NAMED_NUMBER_DICTIONARY as any)[si];
    ...
    return (
        ...
        >
            ...
            {time && <span className={clsx('badge', (LEVEL_BADGE_MAPPING as any)[level])}>{time}</span>}
            {time && (
                <div>
            ...
            <div>
                <ul>
                    {checks.map((check, idx) => (
                        <li key={idx}>
                            <b>{(check as any).name}</b>: {(check as any).message}
                        </li>
                    ))}
                </ul>
            </div>
