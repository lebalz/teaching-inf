...
const controlBits = (octet: string) => {
    const m = octet.match(/(?<control>1{0,4}0)/) as any;
    if (!m) {
        return 0;
    }
...
const BinDecoder = () => {
    ...
    React.useEffect(() => {
        ...
        if (parseStack) {
            ...
            if (bin.length > 7) {
                const chunks = reversed(bin)
                    .split(/(.{6})/)
                    .filter((o) => o)
                    .map((chnk) => reversed(chnk))
                    .reverse();
                let parts = chunks.length;
                if (chunks[0].length > 7 - parts) {
                    /** first byte starts with ones counting the chunks and a single zero. */
                    parts += 1;
                ...
                const layoutChunks = chunks.map((chnk, idx) => {
                    if (idx === 0) {
                        ...
                        return (
                            <span key={idx}>
                                <span className={styles.ctrlBits}>{start}</span>
                                {chnk.substring(0, 8 - start.length)}{' '}
                            </span>
                        );
                    }
                    return (
                        <span key={idx}>
                            <span className={styles.ctrlBits}>10</span>
                            {chnk}{' '}
                        </span>
                    );
                });
