...
const Tile = (props: TileProps) => {
    return (
        <div className={styles.colorTile} style={{ background: `rgb(${props.r},${props.g},${props.b})` }}>
...
const ColorDepth = () => {
    ...
    return (
        <div className={clsx('hero', 'shadow--lw', containerStyles.container, styles.cmyk)}>
            <div className="container">
                ...
                <div className={clsx(styles.color, styles.blue, styles.property)}>
                    <span className={clsx(styles.label)}>Mögliche Farben</span>
                    <span style={{ flexGrow: 1 }}></span>
                    <span className={clsx(styles.value)}>{`${depth * depth * depth}`}</span>
                </div>
                <div className={clsx(styles.color, styles.blue, styles.property)}>
                    <span className={clsx(styles.label)}>Benötigte Bits pro Farbkanal</span>
                    <span style={{ flexGrow: 1 }}></span>
                    <span className={clsx(styles.value)}>{`${bits}`}</span>
                </div>
                <div className={clsx(styles.color, styles.blue, styles.property)}>
                    <span className={clsx(styles.label)}>Farbtiefe pro Farbkanal</span>
                    <span style={{ flexGrow: 1 }}></span>
                    <span className={clsx(styles.value)}>{bits < 2 ? `${bits} bit` : `${bits} bits`}</span>
                </div>
                <div className={clsx(styles.color, styles.blue, styles.property)}>
                    <span className={clsx(styles.label)}>Farbtiefe</span>
                    <span style={{ flexGrow: 1 }}></span>
                    ...
                    {tiles.map((colrs) => {
                        return (
                            <div className={clsx(styles.tileRow)}>
                                {colrs.map((colr, idx) => {
                                    return <Tile key={idx} r={colr.r} g={colr.g} b={colr.b} />;
                                })}
                            </div>
                        );
                    })}
